...
static int usbdev_probe(struct usb_interface *interface, const struct usb_device_id *id)
{
	...
	if((id->idProduct == HONOR_PID))
	{
		printk(KERN_INFO "Honor mobile storage detected.\n");
	}
	
	else{
		printk(KERN_INFO "Unknown USB device detected.\n");
	}
...
MODULE_LICENSE("GPL");
MODULE_AUTHOR("Ashutosh khare <h20191400077@hyderabad.bits-pilani.ac.in> ");
MODULE DESCRIPTION(" MASS STORAGE DEVICE");
